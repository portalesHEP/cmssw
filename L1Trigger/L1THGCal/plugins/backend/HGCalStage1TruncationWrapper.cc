...
void HGCalStage1TruncationWrapper::convertCMSSWInputs(const std::vector<edm::Ptr<l1t::HGCalTriggerCell>>& fpga_tcs,
  ...
  for (auto& tc : fpga_tcs) {
    ...
    unsigned int rOverZbin = rozBin(roverz, 
				      theConfiguration_.rozMin(),
				      theConfiguration_.rozMax(),
				      theConfiguration_.rozBins());//(std::sqrt(x * x + y * y) / std::abs(z)) * FWfactor_;
    ...
    fpga_tcs_SA.emplace_back(
        true, true, rOverZbin, digi_phi, triggerTools_.layerWithOffset(tc->detId()), digi_energy);
